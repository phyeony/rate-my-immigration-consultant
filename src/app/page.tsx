                  ...
                  <div className="flex flex-wrap gap-2">
                    {LOCATIONS.map((loc) => (
                      <button
                        key={loc}
                        type="button"
                        className={`px-3 py-1 rounded-full border text-sm ${selectedLocations.includes(loc) ? "bg-[#337ab7] text-white border-[#337ab7]" : "bg-white text-gray-700 border-gray-300"}`}
                        onClick={() => toggleChip(loc, selectedLocations, setSelectedLocations)}
                      >
                        {loc}
                      </button>
                    ))}
